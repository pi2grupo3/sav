...
class MotionTrack:
    ...
    def traceTargets(self, dstFrame):         
        index = 0
        for tracker in self.targets:
            frame_gray = cv2.cvtColor(dstFrame, cv2.COLOR_BGR2GRAY)
...
class MOSSE:        
    ...
    def update(self, frame, rate = 0.050):
    ...
    def draw_state(self, vis):
        ...
        ut.draw_str(vis, (x1, y2+16), "Num: " + str(self.num))

    def returnRectPoints(self):
        (x, y), (w, h) = self.pos, self.size
        x1, y1, x2, y2 = int(x-0.5*w), int(y-0.5*h), int(x+0.5*w), int(y+0.5*h)
        return x1,y1, x2,y2
